...
class MiConexion:
    ...
    def anadirReclamacion(self, descripcionReclamacion, archivoReclamacion):
        cur = self.conexion.cursor()
        ...
        try:
            ...
            query_reclamacion = """
                ...
                VALUES (%s, %s)
            """
            cur.execute(query_reclamacion, (descripcionReclamacion, archivoReclamacion))
            ...
            # Revertir los cambios si algo falla
            self.conexion.rollback()
            messagebox.showerror("Error", f"Error al realizar la reclamación: {e}")
            print(f"Error al realizar la reclamacións: {e}")

        finally:
            cur.close() 


    def get_reclamaciones(self):
    ...
    def eliminar_reclamacion(self, id):
        cur = self.conexion.cursor()

        try:
            # Consulta para eliminar la reclamación con el ID especificado
            query_eliminar_reclamacion = """
                DELETE FROM reclamaciones
                WHERE idReclamacion = %s
            """
            cur.execute(query_eliminar_reclamacion, (id,))

            # Confirmar los cambios
            self.conexion.commit()
            messagebox.showinfo("Éxito", f"Reclamación con ID {id} eliminada correctamente.")

        except Exception as e:
            # Revertir los cambios si algo falla
            self.conexion.rollback()
            messagebox.showerror("Error", f"Error al eliminar la reclamación: {e}")
            print(f"Error al eliminar la reclamación: {e}")
