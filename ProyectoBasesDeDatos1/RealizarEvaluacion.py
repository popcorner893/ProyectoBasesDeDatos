...
import RealizarEvaluacion,PantallaPerfil
...
class EvaluacionSesionDocente(CTkFrame):
    ...
    def __init__(self, parent, calificado):
        ...
        img_lab1 = CTkButton(panelUsuario, image=iconBack, text="", bg_color="white", fg_color="white", width=51, height=51, hover_color = "white", command = lambda: self.controller.show_frame(RealizarEvaluacion.EvaluacionEnProceso))
        ...
        botonTotal = botonAccion(frameTotal, "", 20, "blanco", 180, 40, lambda: obtenerSuma())
        ...
        downloadIcon = iconoClickable(frameSubirPuntaje, "DownloadIcon2", 35, 35, lambda: mostrarMatriz())
        downloadIcon.pack(side = "left", padx = 20)

        self.botonSubir = botonAccion(frameSubirPuntaje, "Subir Puntaje", 25, "verde", 350, 45, lambda: None)
        self.botonSubir.pack(side = "right", padx = 100)
        ...
        def obtenerSuma():
            try:
                ...
                for i in range(len(requisitos)):  # Iterar sobre el número de requisitos
                    valor = preguntas[i].entradaTexto.get()  # Obtener el valor de la entrada de texto

                    if not valor.strip():  # Verificar si está vacío o contiene solo espacios
                        messagebox.showerror("Error", f"El campo de la pregunta {i + 1} está vacío.")
                        return  # Salir de la función si se encuentra un campo vacío
                    
                    # Intentar convertir el valor a entero o float para la suma
                    suma_total += float(valor)

                # Si pasa todas las validaciones, asignar la suma a self.suma
                self.suma = suma_total*75

                # Mostrar el total en el botón o un mensaje
                botonTotal.configure(text = self.suma)

            except ValueError:
                # Si algún valor no es un número válido
                messagebox.showerror("Error", "Todos los campos deben contener valores numéricos válidos.")

        def mostrarMatriz():
            nueva_Ventana = CTkToplevel(self)
            nueva_Ventana.title("Matriz de Valoración")
            nueva_Ventana.geometry("500x500")
            nueva_Ventana.resizable(False, False)

            icono1 = icono(nueva_Ventana,"matrizSesionDocente",500,500)
            icono1.pack(fill = "both", expand = True)
...
class EvaluacionPropuestaInvestigacion(CTkFrame):
    ...
    def __init__(self, parent, calificado):
        ...
        botonSubTotal = botonAccion(frameSubTotal, "", 20, "blanco", 180, 40, lambda: obtenerSuma1())
        ...
        def obtenerSuma1():
            try:
                ...
                segmentos = [segmento1, segmento2, segmento3, segmento4, segmento5, segmento6, segmento7, segmento8]

                # Iterar por cada segmento
                for segmento in segmentos:
                    # Iterar por cada pregunta dentro del segmento
                    for i, pregunta in segmento.preguntas.items():
                        # Obtener el valor del campo de entrada
                        valor = pregunta.entradaTexto.get()

                        # Verificar si está vacío
                        if not valor.strip():
                            messagebox.showerror("Error", f"El campo de la pregunta en el segmento '{segmento.division.cget('text')}', número {i + 1}, está vacío.")
                            return  # Salir si hay un campo vacío
                        
                        # Intentar convertir el valor a float y sumarlo
                        suma_total += float(valor)

                # Si todo es válido, asignar la suma total a self.suma
                suma_total = (suma_total*15)/140
                self.suma += suma_total

                # Mostrar el total en el botón o actualizar algún widget
                botonSubTotal.configure(text=suma_total)

            except ValueError:
                # Si hay un valor no numérico
                messagebox.showerror("Error", "Todos los campos deben contener valores numéricos válidos.")
        ...
        downloadIcon1 = iconoClickable(frameMatrizSubtotal, "DownloadIcon2", 35, 35, lambda: mostrarMatriz1())
        ...
        def mostrarMatriz1():
            ...
            icono1 = icono(nueva_Ventana,"matrizComponenteEscrito",500,500)
            icono1.pack(fill = "both", expand = True)
        ...
        botonSubTotal1 = botonAccion(frameSubTotal, "", 20, "blanco", 180, 40, lambda: obtenerSuma2())
        ...
        def obtenerSuma2():
            try:
                ...
                for i in range(len(requisitos9)):  # Iterar sobre el número de requisitos
                    valor = preguntasSustentacion[i].entradaTexto.get()  # Obtener el valor de la entrada de texto
                ...
                # Si pasa todas las validaciones, asignar la suma a self.suma
                suma_total = (suma_total*30)/7
                self.suma += suma_total

                # Mostrar el total en el botón o un mensaje
                botonSubTotal1.configure(text = suma_total)
        ...
        downloadIcon2 = iconoClickable(frameMatrizSubtotal1, "DownloadIcon2", 35, 35, lambda: mostrarMatriz2())
        ...
        def mostrarMatriz2():
            ...
            icono1 = icono(nueva_Ventana,"matrizSustentacion",500,500)
            icono1.pack(fill = "both", expand = True)
        ...
        botonTotal = botonAccion(frameTotal, "", 20, "blanco", 180, 40, lambda: calcularTotalFinal())
        botonTotal.pack(side = "right", padx = 100)

        self.botonSubir = botonAccion(panelScrollable, "Subir Puntaje", 25, "verde", 350, 45, lambda: None)
        self.botonSubir.pack(side = "right", padx = 100)

        def calcularTotalFinal():
            if self.suma != 0:
    ...
    def __init__(self, parent, calificado):
        ...
        def obtenerSuma():
            try:
                ...
                # Si todo es válido, asignar la suma total a self.suma
                suma_total = suma_total+240
                self.suma += suma_total

                # Mostrar el total en el botón o actualizar algún widget
                botonTotal.configure(text=suma_total)
        ...
        def mostrarMatriz():
            ...
            icono1 = icono(nueva_Ventana,"matrizHojaDeVida",500,500)
            icono1.pack(fill = "both", expand = True)
...
class EvaluacionEntrevista(CTkFrame):
    ...
    def __init__(self, parent, calificado):
        ...
        for i, requisito in enumerate(requisitos):
            ...
            pregunta = preguntaCuestionario(panelScrollable, requisito, 5)
            pregunta.pack(anchor = "nw", fill = "x", pady = 10)
        ...
        def mostrarMatriz():
            ...
            icono1 = icono(nueva_Ventana,"matrizEntrevista",500,500)
            icono1.pack(fill = "both", expand = True)
